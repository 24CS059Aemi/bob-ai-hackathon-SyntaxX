...
const FEATURES = [
  ...
  },
  {
    ...
    title: 'IEEE C57 / IEC Diagnostic Standards',
    desc: 'Automated diagnostic threshold checking against IEEE C57.91 thermal limits and IEC 60270 partial discharge tolerances to prevent sudden catastrophic failures.',
  },
  {
    icon: (
...
const WORKFLOW = [
  ...
  { step: '04', title: 'Dispatch', body: 'Automated greedy crew pre-positioning and rapid response dispatch with zero manual latency.' },
]
    ...
    <div className="min-h-screen bg-white text-gray-900 antialiased">
      ...
      <header className="fixed top-0 inset-x-0 z-50 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 h-[60px] flex items-center justify-between">
          ...
          {/* CTA Buttons - Always display Sign In, Register, Open Dashboard */}
          <div className="hidden md:flex items-center gap-3">
            <button
              onClick={() => navigate('/login')}
              className="text-sm text-gray-700 hover:text-black font-semibold transition-colors px-3 py-1.5">
              Sign In
            </button>
            <button
              onClick={() => navigate('/register')}
              className="px-3.5 py-1.5 text-xs font-semibold bg-gray-100 hover:bg-gray-200 text-gray-900 rounded-lg border border-gray-300 transition">
              Register
            </button>
            <button
              onClick={() => navigate('/login')}
              className="px-4 py-2 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-700 transition-colors shadow-xs">
              Open Dashboard
            </button>
          </div>
          ...
          <div className="md:hidden border-t border-gray-100 bg-white px-6 py-4 flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-2 mt-1">
              <button onClick={() => navigate('/login')}
                className="py-2 bg-gray-100 border border-gray-300 text-gray-800 text-xs font-semibold rounded-lg text-center">
                Sign In
              </button>
      ...
      <section className="pt-32 pb-20 px-6 bg-white">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <button onClick={() => navigate('/login')}
              className="w-full sm:w-auto px-7 py-3 bg-white text-gray-900 border border-gray-200 hover:bg-gray-50 hover:border-gray-300 font-semibold rounded-xl text-[15px] transition-all shadow-sm flex items-center justify-center gap-2">
            ...
            <button onClick={() => navigate('/register')}
              className="w-full sm:w-auto px-7 py-3 bg-gray-900 hover:bg-gray-800 text-white font-semibold rounded-xl text-[15px] transition-all shadow-sm flex items-center justify-center gap-2">
            ...
            <a href="https://github.com/24CS059Aemi/bob-ai-hackathon-SyntaxX"
              ...
              className="w-full sm:w-auto px-6 py-3 border border-gray-200 text-gray-600 font-medium rounded-xl text-[15px] hover:bg-gray-50 transition-colors text-center">
              View on GitHub →
            </a>
          </div>
      ...
      <section className="py-24 px-6 bg-gray-950 text-white">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button onClick={() => navigate('/login')}
              className="w-full sm:w-auto px-8 py-4 bg-blue-600 text-white font-bold rounded-xl text-[15px] hover:bg-blue-700 transition-all shadow-lg flex items-center justify-center gap-2">
            ...
            <button onClick={() => navigate('/register')}
              className="w-full sm:w-auto px-8 py-4 bg-white/10 text-white border border-white/20 font-bold rounded-xl text-[15px] hover:bg-white/20 transition-all flex items-center justify-center gap-2">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
              </svg>
              Register New Account
      ...
      <footer className="bg-gray-950 border-t border-white/5 text-gray-500 py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs">
          ...
          <div className="flex items-center gap-6">
            <button onClick={() => navigate('/login')} className="hover:text-white transition-colors cursor-pointer bg-transparent border-none text-gray-400 text-xs font-semibold">
              Sign In
            </button>
            <button onClick={() => navigate('/register')} className="hover:text-white transition-colors cursor-pointer bg-transparent border-none text-gray-400 text-xs font-semibold">
              Register
            </button>
            <button onClick={() => navigate('/login')} className="hover:text-white transition-colors cursor-pointer bg-transparent border-none text-gray-400 text-xs font-semibold">
              Dashboard
