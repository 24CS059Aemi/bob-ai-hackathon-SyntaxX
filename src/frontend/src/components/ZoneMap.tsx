...
export default function ZoneMap({ zones, onSelectZone }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-4 border border-black/10">
      <h2 className="text-lg font-bold text-black mb-3">🗺️ Zone Risk Heatmap</h2>
          ...
          return (
            <g key={zone} onClick={() => onSelectZone?.(zone)} className="cursor-pointer">
              ...
              <text x={pos.x + pos.w / 2} y={pos.y + 30} textAnchor="middle"
                fontSize="13" fontWeight="bold" fill="#111111">
                {zone}
              </text>
      ...
      <div className="flex gap-3 mt-2 flex-wrap text-xs">
        {Object.entries(ZONE_FILL).map(([label, color]) => (
          <span key={label} className="flex items-center gap-1 text-gray-700">
