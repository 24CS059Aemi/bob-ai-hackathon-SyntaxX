...
export default function SensorSparklines({ assetId }: Props) {
  ...
  if (loading) return <div className="p-4 text-gray-500 text-sm">Loading sensor data…</div>
  if (!data.length) return <div className="p-4 text-gray-500 text-sm">No sensor data available.</div>
  ...
  const charts: Array<{ key: string; color: string; alarm: number; unit: string }> = [
    { key: 'Temp °C',         color: '#111111', alarm: 85,  unit: '°C' },
    { key: 'Vibration',       color: '#575757', alarm: 3.5, unit: 'mm/s' },
    { key: 'Part. Discharge', color: '#000000', alarm: 150, unit: 'pC' },
    { key: 'Oil Quality',     color: '#777777', alarm: 60,  unit: '' },
  ]

  return (
    <div className="bg-white rounded-2xl shadow p-4 border border-black/10">
      <h2 className="text-lg font-bold text-black mb-1">📈 Sensor Trends — {assetId} (7 days)</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {charts.map(({ key, color, alarm, unit }) => {
          ...
          return (
            <div key={key} className={`rounded-xl p-3 ${isAlarm ? 'bg-zinc-100 border border-black/20' : 'bg-zinc-50 border border-black/10'}`}>
              <div className="flex justify-between items-center mb-1">
                <span className="text-xs font-semibold text-gray-700">{key}</span>
                <span className={`text-xs font-mono font-bold ${isAlarm ? 'text-black' : 'text-gray-700'}`}>
